...
extern "C" {
	...
	MSFS_CALLBACK bool Compass_gauge_callback(FsContext ctx, int service_id, void* pData)
	{
		...
		case PANEL_SERVICE_POST_INSTALL:
		{
			...
			// --- NEW: Create MapView ---
			g_MapViewId = fsMapViewCreate(ctx, 400, 400, FS_RENDER_IMAGE_FLIPY);
			if (g_MapViewId < 0)
				printf("MapView creation FAILED!\n");
			else
				printf("MapView created. ID=%d\n", g_MapViewId);

			// set initial view
			fsMapViewSet2DViewLatLong(g_MapViewId, 0.0, 0.0, false);

			fsMapViewSet2DViewRadiusInMeters(g_MapViewId, 5000.0, false);
		...
		{
			printf("PRE_DRAW fired. MapViewId=%d\n", g_MapViewId);
